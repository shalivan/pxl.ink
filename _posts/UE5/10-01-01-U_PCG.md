...
Data driven Scriptable procedural content framework.  Spawn, filter and instance meshes on points. We can **cache** and reuse calculations. Dependences: you can remove points from bigger assembly. You can spawn on level or on runtime. Smaller detail (without collision whitch is expensive to spawn) should be spawned Runtime to not overload RAM. 
