...
####  Packed Folders
structure - with proper names and types. 
clearly define elemets to operate on them explicitly

`Pack folder SOP` name: `Base` (defauolt naming conv), Type: `skel` (can choose from list ) 
		1. `Base.rig` (rig)
		2. `Base.skel` (skeleton)
		3. `Base.shp` (shape)
		4. `Guides.skel`
...
#### Auto rig Component 
Adding and manipulating graph
At begining nothing will show up, because we need to define what we want
